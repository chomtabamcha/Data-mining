...
round(runif(10,min=1,max=100),0)


# Print 10 random number between 1 and 100 using loop
for ( i in 1:10){
  cat (i, "-->", round(runif(1,min=1,max=100)), "\n")
}
...
#-------------------------------------------------------------
# Help: If you face error in file reading then 
...
#-------------------------------------------------------------
# Step 6.5: Basics of Dataset and Data frame
#-------------------------------------------------------------

NRows=100

NCols=3
p1<- matrix(runif(NCols*NRows, min=1, max=10), ncol=NCols)
head(p1)

NCols=4
p2<- matrix(runif(NCols*NRows), ncol=NCols)
head(p2)

NCols=3
p3<- matrix(runif(NCols*NRows, min=100, max=200), ncol=NCols)
head(p3)

# Merging matrices
dataset<-data.frame(p1,p2,p3)
dataset

head(dataset,2)
names(dataset)

NCols=ncol(p1) + ncol(p2) + ncol(p3)
NCols
